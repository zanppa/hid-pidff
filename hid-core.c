...
//	printk("Irq in\n");
	...
	case 0:			/* success */
		...
		usbhid->retry_delay = 0;
		hid_dbg(hid, "Hid irq in success\n");
		error = hid_input_report(urb->context, HID_INPUT_REPORT,
				 urb->transfer_buffer,
				 urb->actual_length, 1);
...
static void hid_irq_out(struct urb *urb)
{
	...
	hid_dbg(hid, "Output irq\n");
	//printk("Output irqirq\n");
...
static void hid_ctrl(struct urb *urb)
{
	...
	switch (status) {
	case 0:			/* success */
		//printk("Hid ctrl success\n");
		if (usbhid->ctrl[usbhid->ctrltail].dir == USB_DIR_IN) {
		//	printk("Hid ctrl dir in\n");
		//	for (i=0;i<urb->actual_length; i++)
		//		printk("%x ", ((char *)urb->transfer_buffer)[i]);
		//	printk("\n");
...
static void usbhid_disconnect(struct usb_interface *intf)
{
	...
	if (WARN_ON(!hid))
		return;
/*
	if (hid_pidff_destroy)
		hid_pidff_destroy(hid);
*/
